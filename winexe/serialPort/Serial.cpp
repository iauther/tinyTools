...
RBuffer m_rb;
#define MYLEN 10000
BYTE my_buffer[MYLEN];

FILE* hf;
#define FILE_MAXLEN     (100*1000)
static int file_len = 0;
static void fWrite(void *data, DWORD len)
{
    static int f_init = 0;
    if (f_init==0) {
        hf = fopen("./my_test.bin", "wb");
        f_init = 1;
    }

    if (hf) {
        fwrite(data, 1, len, hf);
        file_len += len;
...
int CSerial::Read(void* lpBuffer, DWORD len, DWORD dwWait)
{
    ...
    if (!ReadFile(m_hComm, my_buffer, MYLEN, &dwRead, &m_ReadOverlapped)) {
    ...
    m_rb.write(my_buffer, dwRead);
    rlen = m_rb.read(lpBuffer, len);
    return rlen;
